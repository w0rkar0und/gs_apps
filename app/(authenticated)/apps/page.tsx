          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {visibleApps.map((app) => (
              <Link
                key={app.slug}
                href={app.basePath}
                className="group bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6 hover:shadow-md hover:border-slate-300 hover:-translate-y-0.5 transition-all duration-200"
              >
                <div className="mb-4">
                  <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center group-hover:bg-blue-50 transition-colors duration-200">
                    {APP_ICONS[app.icon] ?? APP_ICONS.generic}
                  </div>
                </div>
                <h2 className="text-base font-semibold text-slate-900 group-hover:text-blue-600 transition-colors duration-200">
                  {app.name}
                </h2>
                <p className="text-sm text-slate-500 mt-1 leading-relaxed">{app.description}</p>
              </Link>
            ))}
          </div>
        )}
