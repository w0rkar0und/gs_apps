...
export async function POST(request: NextRequest) {
  ...
  // Check reports access via user_apps or admin
  ...
  if (!profile?.is_admin) {
    ...
    if (!perms?.['working-days']) {
      return NextResponse.json({ error: 'You do not have access to the Contractor - Working Day Count Report.' }, { status: 403 })
    }
  }
  ...
  try {
    const proxyRes = await fetch(`${process.env.RAILWAY_PROXY_URL}/report/working-days`, {
      method: 'POST',
      headers: {
