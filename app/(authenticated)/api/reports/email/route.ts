...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (reportType === 'deposit') {
      ...
      subject = `Contractor - Working Day Count — ${hrCode} ${contractorName}`
      ...
      subject = `DA Relations Settlement Data — ${hrCode} ${contractorName}`
      ...
      subject = `Branch/Client Performance — Weeks ${first?.week ?? 0}–${last?.week ?? 0}, ${last?.year ?? 0}`
    } else {
    ...
    const { error: sendError } = await resend.emails.send({
      ...
      html: reportType === 'working-days-by-client'
        ...
        ? `<p>Please find attached the Branch/Client Performance report for Weeks ${reportData.weeks?.[0]?.week ?? 0}–${reportData.weeks?.[reportData.weeks.length - 1]?.week ?? 0}, ${reportData.weeks?.[reportData.weeks.length - 1]?.year ?? 0}.</p><p>Generated by ${profile?.display_id ?? 'GS Apps'} on ${new Date().toLocaleDateString('en-GB')}.</p>`
        : `<p>Please find attached the ${reportType === 'deposit' ? 'Deposit Report' : reportType === 'settlement' ? 'DA Relations Settlement Data' : 'Contractor - Working Day Count Report'} for <strong>${hrCode} — ${contractorName}</strong>.</p><p>Generated by ${profile?.display_id ?? 'GS Apps'} on ${new Date().toLocaleDateString('en-GB')}.</p>`,
      attachments: [
        {
          filename,
