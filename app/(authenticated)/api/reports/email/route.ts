...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (reportType === 'deposit') {
      ...
      subject = `Working Day Count — ${hrCode} ${contractorName}`
    } else {
    ...
    const { error: sendError } = await resend.emails.send({
      ...
      to: recipientEmail,
      subject,
      html: `<p>Please find attached the ${reportType === 'deposit' ? 'Deposit Report' : 'Working Day Count Report'} for <strong>${hrCode} — ${contractorName}</strong>.</p><p>Generated by ${profile?.display_id ?? 'GS Apps'} on ${new Date().toLocaleDateString('en-GB')}.</p>`,
      attachments: [
        {
          filename,
